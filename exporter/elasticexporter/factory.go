...
// Factory is the factory for Elastic APM exporter.
type Factory struct{}
...
func (f *Factory) OCAgentOptions(logger *zap.Logger, ocac *Config) ([]ocagent.ExporterOption, error) {
	if ocac.Endpoint == "" {
		return nil, &ocExporterError{
			...
			msg:  "Elastic APM exporter config requires an Endpoint",
		}
	}
	...
	if ocac.Compression != "" {
		if compressionKey := compressiongrpc.GetGRPCCompressionKey(ocac.Compression); compressionKey != compression.Unsupported {
			...
			return nil, &ocExporterError{
				...
				msg:  fmt.Sprintf("Elastic APM exporter unsupported compression type %q", ocac.Compression),
			}
		}
	}
	if ocac.CertPemFile != "" {
		...
		if err != nil {
			return nil, &ocExporterError{
				...
				msg:  fmt.Sprintf("Elastic APM exporter unable to read TLS credentials from pem file %q: %v", ocac.CertPemFile, err),
			}
		}
		...
		if err != nil {
			return nil, &ocExporterError{
				code: errUnableToGetTLSCreds,
				msg: fmt.Sprintf(
					"Elastic APM exporter unable to read certificates from system pool: %v", err),
			}
		}
...
func (f *Factory) CreateMetricsExporter(logger *zap.Logger, config configmodels.Exporter) (exporter.MetricsExporter, error) {
	// TODO(axw) support sending metrics to APM Server
